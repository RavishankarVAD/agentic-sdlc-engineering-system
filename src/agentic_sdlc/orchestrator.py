...
class WorkflowOrchestrator:
    """Coordinates agents, dependency-aware execution, bounded recovery and approval."""
    ...
    def run(self, requirement: str, codebase: str | None = None, approve: bool = False) -> EngineeringOutcome:
        ...
        tasks[0].status = tasks[1].status = "completed"
        log.append("planner-agent: created dependency-aware execution plan")
        ...
        tasks[4].status = "completed" if validation.passed else "failed"
        log.append(f"validation-agent: validation passed={validation.passed}")

        attempts = 0
        while not validation.passed and attempts < self.max_repair_attempts:
            attempts += 1
            log.append(f"repair-agent: bounded repair attempt {attempts}")
            artifacts = self.generator.generate(analysis, architecture)
            validation = self.validator.validate(analysis, artifacts)
        tasks[5].status = "completed" if validation.passed else "blocked"

        approval_status = "approved" if approve and validation.passed else "awaiting_human_approval"
        tasks[6].status = "completed" if approval_status == "approved" else "pending"
